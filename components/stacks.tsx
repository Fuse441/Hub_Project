...
function Stack() {
  ...
  const [current, setCurrent] = useState(1); 
    const stackTitle = ["Frameworks and Libraries","Tools for Development and Collaboration","Programming Languages"]
...
useEffect(() => {
  if (theme) setCurrentTheme(theme);
    ...
    return (
          ...
          <CardBody className='flex flex-row w-[450px] flex-wrap'>
            ...
            .map(((stack,index) => (
                ...
                <div key={index} className={`m-1.5`}>
                 ...
                 radius="none"
                 className={
                    `${(currentTheme == "dark" && stack.background == true)  ? 'invert-[100]' : ''}`
                 }            
                 alt={stack.name}
                 src={stack.icon}
                 width={45}
      ...
      <Pagination showControls onChange={setCurrent} initialPage={current} total={3} loop />
    </div>
      </div>
      )}
